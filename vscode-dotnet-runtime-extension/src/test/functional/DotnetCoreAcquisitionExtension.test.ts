...
suite('DotnetCoreAcquisitionExtension End to End', function() {
  ...
  test('Telemetry Sent During Install and Uninstall', async () => {
    ...
    assert.notInclude(requestedEvent!.properties!.RequestingExtensionId, requestingExtensionId); // assert that the extension id is hashed    const startedEvent = MockTelemetryReporter.telemetryEvents.find((event: ITelemetryEvent) => event.eventName === 'DotnetAcquisitionStarted');
