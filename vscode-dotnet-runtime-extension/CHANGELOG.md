...
- To install a local (user-folder) .NET SDK with .NET Install Tool 3.2.0 or later, call `dotnet.acquire` with `{ mode: 'sdk' }`. Local SDK acquisition accepts a `major.minor` version (resolved to the latest patch) or a fully specified version (e.g. `8.0.408`, installed exactly); major-only and feature band versions remain a `dotnet.acquireGlobalSDK`-only capability. As with runtimes, the `PATH` is not configured for local SDK installs.
