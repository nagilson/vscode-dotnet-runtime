...
export class CommandExecutorSingleton extends ICommandExecutor
{
    ...
    private readonly validSudoCommands: string[] = []
    private static instance: CommandExecutorSingleton;
    ...
    public setValidSudoCommands(commands: string[]): void
    {
        this.validSudoCommands.push(...commands);
    }

    public static getInstance(): CommandExecutorSingleton
    {
        if (!CommandExecutorSingleton.instance)
        {
            CommandExecutorSingleton.instance = new CommandExecutorSingleton();
        }

        return CommandExecutorSingleton.instance;
    }
     ...
     */
    private async ExecSudoAsync(command: CommandExecutorCommand, context: IAcquisitionWorkerContext, utilityContext: IUtilityContext, terminalFailure = true): Promise<CommandExecutorResult>
    {
        const fullCommandString = CommandExecutorSingleton.prettifyCommandExecutorCommand(command, false);
        context?.eventStream.post(new CommandExecutionUnderSudoEvent(`The command ${fullCommandString} is being ran under sudo.`));
        const shellScript = path.join(this.sudoProcessCommunicationDir, 'interprocess-communicator.sh');

        if (await isRunningUnderWSL(context, utilityContext))
        {
...
Please install the .NET SDK manually by following https://learn.microsoft.com/en-us/dotnet/core/install/linux-ubuntu. Then, add it to the path by following https://github.com/dotnet/vscode-dotnet-runtime/blob/main/Documentation/troubleshooting-runtime.md#manually-installing-net`,
        ...
        const masterSudoProcessSpawnResult = this.startupSudoProc(fullCommandString, context, shellScript, terminalFailure);

        await this.sudoProcIsLive(terminalFailure, context);
        return this.executeSudoViaProcessCommunication(fullCommandString, context, terminalFailure);
    }
     ...
     */
    private async startupSudoProc(fullCommandString: string, context: IAcquisitionWorkerContext, shellScriptPath: string, terminalFailure: boolean): Promise<string>
    {
        if (this.hasEverLaunchedSudoFork)
        {
            if (await this.sudoProcIsLive(false, context))
            {
        ...
        context?.eventStream.post(new CommandExecutionUserAskDialogueEvent(`Prompting user for command ${fullCommandString} under sudo.`));

        const options = { name: this.getSanitizedCallerName(context) };

        fs.chmodSync(shellScriptPath, 0o500);
        const timeoutSeconds = Math.max(100, context.timeoutSeconds);
        ...
        {
            context?.eventStream.post(new CommandExecutionStdOut(`The process spawn: ${fullCommandString} encountered stdout, continuing
${stdout}`));

            context?.eventStream.post(new CommandExecutionStdError(`The process spawn: ${fullCommandString} encountered stderr, continuing
${stderr}`));

            if (error)
            {
                context?.eventStream.post(new CommandExecutionUserCompletedDialogueEvent(`The process spawn: ${fullCommandString} failed to run under sudo.`));
                if (terminalFailure)
                {
                    this.parseVSCodeSudoExecError(error, context, fullCommandString);
            ...
            {
                context?.eventStream.post(new CommandExecutionUserCompletedDialogueEvent(`The process spawn: ${fullCommandString} successfully ran under sudo.`));
                return Promise.resolve('0');
            }
        });
     ...
     */
    private async sudoProcIsLive(errorIfDead: boolean, context: IAcquisitionWorkerContext): Promise<boolean>
    {
        let isLive = false;
        ...
        await (this.fileUtil as FileUtilities).writeFileOntoDisk('', fakeLockFile, false, context?.eventStream);
        ...
        await lockfile.lock(fakeLockFile, { lockfilePath: directoryLockPath, retries: { retries: 10, minTimeout: 5, maxTimeout: 10000 } })
            ...
            {
                context?.eventStream.post(new DotnetLockAcquiredEvent(`Lock Acquired.`, new Date().toISOString(), directoryLockPath, fakeLockFile));

                await (this.fileUtil as FileUtilities).wipeDirectory(this.sudoProcessCommunicationDir, context?.eventStream, ['.txt']);

                await (this.fileUtil as FileUtilities).writeFileOntoDisk('', processAliveOkSentinelFile, true, context?.eventStream);
                context?.eventStream.post(new SudoProcAliveCheckBegin(`Looking for Sudo Process Master, wrote OK file. ${new Date().toISOString()}`));

                const waitTime = context?.timeoutSeconds ? ((context?.timeoutSeconds / 3) * 1000) : 180000;
                ...
                context?.eventStream.post(new DotnetLockReleasedEvent(`Lock about to be released.`, new Date().toISOString(), directoryLockPath, fakeLockFile));
                return release();
            });

        context?.eventStream.post(new SudoProcAliveCheckEnd(`Finished Sudo Process Master: Is Alive? ${isLive}. ${new Date().toISOString()}`));
...
It had previously spawned: ${this.hasEverLaunchedSudoFork}.`), getInstallFromContext(context));
            context?.eventStream.post(err);
            throw err.error;
        }
     ...
     */
    private async executeSudoViaProcessCommunication(commandToExecuteString: string, context: IAcquisitionWorkerContext, terminalFailure: boolean, failOnNonZeroExit = true): Promise<CommandExecutorResult>
    {
        ...
        await lockfile.lock(fakeLockFile, { lockfilePath: directoryLockPath, retries: { retries: 10, minTimeout: 5, maxTimeout: 10000 } })
            ...
            {
                context?.eventStream.post(new DotnetLockAcquiredEvent(`Lock Acquired.`, new Date().toISOString(), directoryLockPath, fakeLockFile));
                await (this.fileUtil as FileUtilities).wipeDirectory(this.sudoProcessCommunicationDir, context?.eventStream, ['.txt', '.json']);

                await (this.fileUtil as FileUtilities).writeFileOntoDisk(`${commandToExecuteString}`, commandFile, true, context?.eventStream);
                context?.eventStream.post(new SudoProcCommandExchangeBegin(`Handing command off to master process. ${new Date().toISOString()}`));
                context?.eventStream.post(new CommandProcessorExecutionBegin(`The command ${commandToExecuteString} was forwarded to the master process to run.`));


                const waitTime = context?.timeoutSeconds ? (context?.timeoutSeconds * 1000) : 600000;
                await loopWithTimeoutOnCond(100, waitTime,
        ...
        context?.eventStream.post(new SudoProcCommandExchangeEnd(`Finished or timed out with master process. ${new Date().toISOString()}`));
...
It had previously spawned: ${this.hasEverLaunchedSudoFork}.`), getInstallFromContext(context));
        ...
        {
            context?.eventStream.post(new CommandProcessesExecutionFailureNonTerminal(`The command ${commandToExecuteString} never finished under the process, but it was marked non terminal.`));
        }
        else
        {
            context?.eventStream.post(new CommandProcessorExecutionEnd(`The command ${commandToExecuteString} was finished by the master process, as ${outputFile} was found.`));

            context?.eventStream.post(new CommandExecutionStdOut(`The command ${commandToExecuteString} encountered stdout, continuing
${(commandOutputJson as CommandExecutorResult).stdout}`));

            context?.eventStream.post(new CommandExecutionStdError(`The command ${commandToExecuteString} encountered stderr, continuing
...
${(commandOutputJson as CommandExecutorResult).stderr}.`),
     ...
     */
    public async endSudoProcessMaster(context: IAcquisitionWorkerContext): Promise<number>
    {
        ...
        await (this.fileUtil as FileUtilities).writeFileOntoDisk('', processExitFile, true, context?.eventStream);
        ...
        {
            context.eventStream.post(new TriedToExitMasterSudoProcess(`Tried to exit sudo master process: FAILED. ${error ? JSON.stringify(error) : ''}`));
        }

        context.eventStream.post(new TriedToExitMasterSudoProcess(`Tried to exit sudo master process: exit code ${didDelete}`));

        return didDelete;
    }

    public async executeMultipleCommands(commands: CommandExecutorCommand[], context: IAcquisitionWorkerContext, utilityContext: IUtilityContext, options?: any, terminalFailure = true): Promise<CommandExecutorResult[]>
    {
        const results = [];
        for (const command of commands)
        {
            results.push(await this.execute(command, context, utilityContext, options, terminalFailure));
        }
     ...
     */
    public async execute(command: CommandExecutorCommand, context: IAcquisitionWorkerContext, utilityContext: IUtilityContext, options: any = null, terminalFailure = true): Promise<CommandExecutorResult>
    {
        ...
        {
            useCache = true;
            const cachedResult = LocalMemoryCacheSingleton.getInstance().getCommand({ command, options }, context);
        ...
        {
            const sudoResult = await this.ExecSudoAsync(command, context, utilityContext, terminalFailure);
            if (useCache)
            {
                LocalMemoryCacheSingleton.getInstance().putCommand({ command, options }, sudoResult, context);
            }
            return sudoResult;
        }
        else
        {
            context?.eventStream.post(new CommandExecutionEvent(`Executing command ${fullCommandString}
with options ${JSON.stringify(options)}.`));
            ...
            if (command.runUnderSudo)
            {
                // Remove this when https://github.com/typescript-eslint/typescript-eslint/issues/2728 is done
                // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
                options.name = this.getSanitizedCallerName(context);
                return new Promise<CommandExecutorResult>((resolve, reject) =>
                {
                    execElevated(fullCommandString, options, (error?: Error, execStdout?: string | Buffer, execStderr?: string | Buffer) =>
                    {
                        ...
                        {
                            return reject(this.parseVSCodeSudoExecError(error, context, fullCommandString));
                        }
                        else if (error)
                        {
                            context?.eventStream.post(new CommandExecutionStdError(`The command ${fullCommandString} encountered ERROR: ${JSON.stringify(error)}`));
                        }

                        const result = { status: error ? error.message : '0', stderr: execStderr, stdout: execStdout } as CommandExecutorResult
                        if (useCache)
                        {
                            LocalMemoryCacheSingleton.getInstance().putCommand({ command, options }, result, context);
                        }
            ...
            const commandResult: CommandExecutorResult = await promisify(proc.exec)(fullCommandString, options).then(
                ...
                { // see https://nodejs.org/api/child_process.html#child_processexeccommand-options-callback
                    ...
                    if (terminalFailure)
                    {
                        this.logCommandResult(result, context, fullCommandString);
            ...
            this.logCommandResult(commandResult, context, fullCommandString);

            if (useCache)
            {
                LocalMemoryCacheSingleton.getInstance().putCommand({ command, options }, commandResult, context);
            }
            return commandResult;
        }
    }

    private logCommandResult(commandResult: CommandExecutorResult, context: IAcquisitionWorkerContext, fullCommandStringForTelemetryOnly: string)
    {
        context?.eventStream.post(new CommandExecutionStatusEvent(`The command ${fullCommandStringForTelemetryOnly} exited:
${commandResult.status}.`));

        context?.eventStream.post(new CommandExecutionStdOut(`The command ${fullCommandStringForTelemetryOnly} encountered stdout:
${commandResult.stdout}`));

        context?.eventStream.post(new CommandExecutionStdError(`The command ${fullCommandStringForTelemetryOnly} encountered stderr:
${commandResult.stderr}`));
    }

    private parseVSCodeSudoExecError(error: any, context: IAcquisitionWorkerContext, fullCommandString: string): Error
    {
...
The user refused the password prompt.`),
                getInstallFromContext(context));
            context?.eventStream.post(cancelledErr);
...
Please report this at https://github.com/dotnet/vscode-dotnet-runtime/issues.`),
                getInstallFromContext(context));
            context?.eventStream.post(securityErr);
     ...
     */
    public async tryFindWorkingCommand(commands: CommandExecutorCommand[], context: IAcquisitionWorkerContext, utilityContext: IUtilityContext, options?: any): Promise<CommandExecutorCommand | null>
    {
        ...
        for (const command of commands)
        {
            ...
            {
                // eslint-disable-next-line @typescript-eslint/no-unsafe-member-access
                const cmdFoundOutput = (await this.execute(command, context, utilityContext, options?.at(optIdx) ?? options, false)).status;
                if (cmdFoundOutput === '0')
                {
                    workingCommand = command;
                    context?.eventStream.post(new DotnetAlternativeCommandFoundEvent(`The command ${command.commandRoot} was found.`));
                    break;
                }
                else
                {
                    context?.eventStream.post(new DotnetCommandNotFoundEvent(`The command ${command.commandRoot} was NOT found, no error was thrown.`));
                }
            }
            catch (err)
            {
                // Do nothing. The error should be raised higher up.
                context?.eventStream.post(new DotnetCommandNotFoundEvent(`The command ${command.commandRoot} was NOT found, and we caught any errors.`));
            }
            ++optIdx;
        }
    ...
    public async setEnvironmentVariable(variable: string, value: string, context: IAcquisitionWorkerContext, utilityContext: IUtilityContext, vscodeContext: IVSCodeExtensionContext, failureWarningMessage?: string, nonWinFailureMessage?: string)
    {
        ...
        {
            const setShellVariable = CommandExecutorSingleton.makeCommand(`set`, [`${variable}=${value}`]);
            const setSystemVariable = CommandExecutorSingleton.makeCommand(`setx`, [`${variable}`, `"${value}"`]);
            try
            {
                const shellEditResponse = (await this.execute(setShellVariable, context, utilityContext, null, false)).status;
                environmentEditExitCode += Number(shellEditResponse[0]);
                const systemEditResponse = (await this.execute(setSystemVariable, context, utilityContext, null, false)).status
        ...
        {
            utilityContext.ui.showWarningMessage(failureWarningMessage, () => {/* No Callback */ },);
        }
    }
    ...
    private getSanitizedCallerName(context: IAcquisitionWorkerContext): string
    {
        // The '.' character is not allowed for sudo-prompt so we use 'NET'
        let sanitizedCallerName = context?.acquisitionContext?.requestingExtensionId?.replace(/[^0-9a-z]/gi, ''); // Remove non-alphanumerics per OS requirements
