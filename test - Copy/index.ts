...
export class INodeIPCMutexLogger
{
    ...
    {
        console.log(...args); // Replace with your own logging implementation.
    }
}
...
export class NodeIPCMutex
{
    ...
    private release(): void
    {
        ...
        {
            this.server.close();
            // TODO: Should we delete the IPC Handle on Unix or does that happen when the processs is not killed?
    ...
    private async isLockStale(msg: string): Promise<boolean>
    {
        const resolved = await new Promise<boolean>((resolve, reject) =>
        {
            ...
            {
                // Reject and throw if we can't connect to the lock. Handled by the catch below.
                this.logger.log(`Unable to connect to existing lock: ${JSON.stringify(err ?? '')}.`);
                return resolve(true); // Possible error: ENOENT, if the other process finishes while we wait. It's ok if we consider it stale now, even if it doesnt exist.
            });
        })
            .catch((error: any) => // Handle synchronous errors from the socket connection.
            {
                ...
                throw (error); // We don't know what happened, but we can't acquire the lock.
            });
        return resolved;
    }
...
async function printWithLock(msg: string, waitTimeMs: number)
{
    const mutex = new NodeIPCMutex('example-lock');

    const c = await mutex.acquire(async () =>
    {
        console.log(`Acquired lock: ${msg}`);
        await new Promise((resolve) => setTimeout(resolve, waitTimeMs));
        console.log(`Releasing lock: ${msg}`);
        return msg;
    }, 100, 10000, msg);

    console.log(`After release, ${msg} returned c: ${c}`);
}

class INodeIPCTestLogger extends INodeIPCMutexLogger
{
    public logs = [];
...
async function main()
{
    ...
    printWithLock('Task A', 2000);
    printWithLock('Task B', 2000);
}
